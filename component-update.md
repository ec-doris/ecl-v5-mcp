...
- Status: ready
- Phase: none
- Last completed: `content-block`
- Next component: `content-item`
- Next action: change `content-item` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Content-block completed 2026-07-14. `components/content-block.html.twig` is byte-identical to tagged v5.0.1 source with SHA-256 `9d64a76a895ea0bef000683024fd078404b3c4f62946ff0139cafe3022f1b827`; reusable HTML SHA-256 `5bfd12b641135333d8912be5e772d519782fd28a20f3cd2ddf90fbf97ab2be0e` has no Twig and covers labels, primary meta, title string and link with injected title hook, description, vertical and horizontal secondary meta, icon present/absent, delegated description-list shapes and list extra class, flat and grouped links, plain link items, root/link extension attributes, root-only empty output, runtime hooks, and all 15 non-default EC color modes. Runtime initializes only when a related `[data-ecl-picture-link]` is found; content-block owns no picture markup, so examples preserve hooks but do not invent picture output. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, focused content-block hook/coverage assertions, package dry-run inclusion, JS syntax check, Jest smoke with `--passWithNoTests`, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
