...
- Status: ready
- Phase: none
- Last completed: `site-footer`
- Next component: `site-header`
- Next action: change `site-header` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Site footer completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the site-footer start checkpoint edit. Tagged site-footer tree lists README, package metadata, three component-owned Twig templates, EC and EU SCSS, print SCSS, EC and EU stories, EC/EU demo data for core, harmonised, and standardised variants, tests, and snapshots; no EC website docs were listed by the initial tree command. Twig sync passed for `site-footer-ec.html.twig`, `site-footer-ec-section.html.twig`, and `site-footer-eu.html.twig`. Audit found no site-footer JavaScript runtime or auto-init hook; compiled JS contains no site-footer hook, while compiled CSS/print CSS contain EC footer selectors and print hides `.ecl-site-footer`. The EC preset CSS does not include EU-only selector rules such as `ecl-site-footer--split-columns`, `ecl-site-footer__list--separator`, `ecl-site-footer__list--columns`, or `ecl-site-footer__title--separator`; those branches remain covered in HTML because the component-owned EU Twig is part of this inventory row. HTML covers EC co-owner, feedback, site-info, core, fallback links, custom batches, common social/extra/inline links, root and section extension hooks, tagged empty EC output, EU rows/columns/logo/title/content/list/action branches, tagged empty EU output, no-runtime evidence, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion, focused site-footer selector/branch/no-runtime checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
