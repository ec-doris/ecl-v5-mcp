...
- Status: ready
- Phase: none
- Last completed: `skip-link`
- Next component: `slogan-ticker`
- Next action: change `slogan-ticker` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Skip-link completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the skip-link start checkpoint edit. Tagged skip-link tree lists README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, demo data, test, and snapshot; no JavaScript runtime or EC website docs were listed by the initial tree command. Upstream audit completed: README exposes `label`, `href`, `extra_classes`, and `extra_attributes`; story only adds a focus state outside rendered markup; demo data is `label: Skip to main content` and `href: #top`; tests/snapshots cover default, extra classes, and valued extra attributes; Twig delegates to `@ecl/link/link.html.twig` with `type: primary`; link v5.0.1 renders this usage as a plain `<a>` with `href`, `ecl-link ecl-link--primary ecl-skip-link`, optional extra classes, optional valued/valueless attributes, and text. SCSS positions the link and hides it except `:active`/`:focus-visible`; print CSS hides it. Compiled EC CSS contains `ecl-skip-link` and `ecl-link--primary`; compiled JS has no skip-link selector or auto-init name. No EC website docs were present. HTML covers default, empty output, extension classes, valued/valueless extension attributes, no authored runtime `data-ecl-auto-initialized`, no runtime hook, and all 15 non-default EC color modes. Twig sync passed after preserving the upstream file's extra terminal blank line. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion, focused selector/runtime checks, JS syntax checks for `index.js` and both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
