...
- Next component: `date-block`
- Next action: change `date-block` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Content-item completed 2026-07-14. `components/content-item.html.twig` is byte-identical to tagged v5.0.1 source with SHA-256 `d833ca54376904d879deb6435587033f286d56918b83989acd88fdbcdb36d5d8`; reusable HTML covers default delegated content-block output, labels and labels aria, primary meta, linked and plain titles, description, secondary meta with and without icons, vertical and horizontal meta, description-list variants, picture left/right/top, small/large picture classes, responsive source, zoom, clickable picture hook present/absent, date-block inline event output, divider, root extra class, valued and valueless attributes, tagged empty-input root plus empty content-block output, nested ContentBlock runtime hooks, picture/title link runtime hooks, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, focused content-item class/hook/coverage assertions, package dry-run inclusion, JS syntax check, Jest smoke with `--passWithNoTests`, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. The pinned ESM bundle exposes `ContentBlock`, `data-ecl-picture-link`, and `data-ecl-title-link`; unlike the UMD bundle it does not contain `data-ecl-content-block` as a literal selector string, so focused runtime verification checked the selector literals against the UMD bundle and the component class against both bundles.
