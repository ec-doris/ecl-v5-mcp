...
- Status: ready
- Phase: none
- Last completed: `list-illustration`
- Next component: `media-container`
- Next action: change `media-container` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: List illustration completed 2026-07-15. Preflight passed: outer tree and linked checkout status outputs were clean, `europa-component-library` is a symlink, and tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`. Tagged inventory has README, package metadata, two component-owned Twig templates, SCSS, print SCSS, story, four demo data files, tests, snapshots, and EC website usage/code/accessibility pages. Twig sync passed for `components/list-illustration.html.twig` and `components/list-illustration-item.html.twig`. Reusable HTML covers `ul`, `ol`, zebra, columns 2/3/4, centered, font size `m`, image and square image sizes `s`/`m`/`l`, decorative and accessible icons, icon size mapping, icon inline, icon-list, divider true/false, number-list reset and no-reset branches, custom counter start, title/value/description/empty detail branches, root and item extension hooks, tagged empty/non-iterable no-root output, no component runtime hooks, delegated icon and picture output, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, focused selector/list/runtime checks, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
