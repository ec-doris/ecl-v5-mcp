...
- Status: ready
- Phase: none
- Last completed: `rating-field`
- Next component: `search-form`
- Next action: change `search-form` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Rating field completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged package contains README, package metadata, one component-owned Twig template, SCSS, print SCSS, demo data, tests, and snapshots; no JavaScript runtime or EC website docs were listed by the initial tree command. Audit found a single component-owned Twig file with icon imports only; local Twig already matched the tag byte-for-byte. Twig defines `id`, `items`, `name`, `disabled`, `invalid`, `star_filled_icon`, `star_outline_icon`, `required`, `extra_classes`, and `extra_attributes`. The root renders only when `items` is a non-empty iterable, reverses items, derives IDs as `<id>-<value>`, prints `checked`, `required`, and `disabled` on inputs, prints `ecl-rating-field--disabled` on the root when disabled, and prints an sr-only label only when item `label` is set. `invalid` is unused by the component-owned Twig; `extra_classes` and `extra_attributes` are computed into `_css_class`/`_extra_attributes` but never printed, so extension hooks intentionally have no static output. Form-group wrappers, invalid messaging, and optional/required label text are delegated to the separate form-group component and are not rating-field-owned output. No runtime hooks or auto-init exist; print CSS hides `.ecl-rating-field`. HTML covers default five-item required output, reversed order, labels, checked, disabled root/input output, minimal no-label/no-name item, tagged empty no-output branch, unused invalid/extension inputs, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused rating-field selector/branch checks against compiled CSS, no runtime hook checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
