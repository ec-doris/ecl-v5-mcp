...
- Status: ready
- Phase: none
- Last completed: `spotlight`
- Next component: `table`
- Next action: change `table` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Spotlight completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer status showed only this pass's tracker change, and linked checkout status output was empty. Tagged source lists README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, EC demo data, test, snapshots, and EC website usage/code/accessibility docs. No component-owned JavaScript exists; no `Spotlight` runtime hook or `data-ecl-spotlight` selector is present in compiled bundles. Dependency is `picture`; local picture Twig is byte-for-byte identical to the tag and renders `data-ecl-spotlight-image` on the `<picture>` root, not the `<img>`. Twig sync passed with byte-for-byte `cmp` against the tag. HTML covers default linked output, responsive and single-image picture output, `data-ecl-spotlight-image` on `<picture>`, `data-ecl-spotlight-container`, font `m`/`l`, full-width, header omitted/present, credit omitted/present, link omitted/present, anchor omitted/present, image-only content edge, extension classes, valued and valueless extension attributes, and all 15 non-default EC color modes. Empty `picture` suppresses all output and story-only sidebar/grid wrappers are intentionally excluded. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused spotlight selector/runtime checks, package dry-run inclusion, JS syntax checks for `index.js` and both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
