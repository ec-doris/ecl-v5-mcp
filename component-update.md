...
- Status: ready
- Phase: none
- Last completed: `page-header`
- Next component: `page-information`
- Next action: change `page-information` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Page-header completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, the linked checkout reported no short-status entries, and the outer worktree only had this checkpoint change before component edits. Tagged package contains README, package metadata, two component-owned Twig templates, expandable JavaScript, EC/EU SCSS and print SCSS, demo data, story, tests, and snapshots; no EC website docs were present under `src/website/src/pages/ec/components/page-header`. Audit found the v5.0.1 Twig removes older `variant`/`font_size` branches and defines `color_mode`, `has_background`, `title`, `hide_title`, `description`, `description_position`, `expandable`, `picture_thumbnail`, `picture_background`, `picture_position`, `meta`, `breadcrumb`, `extra_classes`, and `extra_attributes`. Root classes change for color mode, background, description position only when description exists, picture position when `picture_background` is non-empty, and root extension hooks. Title, description/thumbnail, meta, breadcrumb, and picture render independently. `description_position: bottom` moves description into a bottom section; top renders inside `ecl-page-header__description-top`. `picture_position` top renders before the container, beside renders column markup with `ecl-page-header__section-info` and `ecl-page-header__content-info`, and bottom renders inside stack content after meta. Expandable renders before the picture when provided; with panel content it has `data-ecl-auto-init="PageHeaderExpandable"`, header hook, button toggle hook, `aria-controls`, `aria-expanded="false"`, hidden panel, optional panel id, toggle extra attributes, root extra classes, and root attributes. Tagged expandable Twig contains an `_lists` reference that is not defined before its empty check; the normal expandable branch is covered, and the no-panel branch is recorded as upstream edge behavior only. Tagged story controls mutate only through `prepareData`; raw Storybook booleans like `show_picture` are not component API. Tagged tests/snapshots cover default expandable, hidden title/no meta, description bottom, picture bottom, picture beside, extra root classes, and valued root attributes. Local compiled assets contain page-header selectors and PageHeaderExpandable runtime hooks; top-description/top-picture markup-state classes are emitted by tagged Twig but have no compiled CSS selectors. Twig sync passed for `components/page-header.html.twig` and `components/page-header-expandable.html.twig`. Reusable HTML covers title, hidden title, description top/bottom, thumbnail, breadcrumb, mixed meta with icons, picture top/beside/bottom, background with color mode, expandable runtime hooks, custom panel id, toggle extension attributes, root extra class, valued and valueless root attributes, empty root output, upstream no-panel expandable edge output, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused selector/runtime checks, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
