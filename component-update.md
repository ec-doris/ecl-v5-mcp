...
- Status: ready
- Phase: none
- Last completed: `category-filter`
- Next component: `checkbox`
- Next action: change `checkbox` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Category filter completed 2026-07-14. `components/category-filter.html.twig` and `components/category-filter-items.html.twig` are byte-identical to their tagged v5.0.1 sources, with SHA-256 `c0ce18bb9b2ea4c4d675fc731881e436a5cfce7f84213da157ae2a3a54f70ae7` and `7fb0885d9f44c1f0b4ae8fef54cc62f0cac086bb559c9771fa81c6b96f8b9ee9`; reusable HTML SHA-256 `2b5e8ca168e4df171b2ace2ccebeca306740e0f114bd1a2ff0b7562d843e1e8b` has no Twig and covers the labelled default tree, current link, level 1-4 items, parent button and child link branches, root/item extension hooks with valued and valueless attributes, omitted label, empty items, static auto-init hooks, icon output, and all 15 non-default EC color modes. Runtime-only `data-ecl-auto-initialized`, clicked `aria-current`, `aria-expanded="true"`, and `ecl-category-filter__list-item--open` are intentionally excluded. Category filter imports `icon`; that separate row was not changed. Validation passed: Twig identity for both Twig files, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused hook/runtime-exclusion assertions, JS syntax checks, Jest smoke with `--passWithNoTests`, package dry-run inclusion, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
