...
- Next component: `tabs`
- Next action: change `tabs` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Table completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer status showed no output before this pass's tracker edit, and linked checkout status output was empty. Tagged source lists README, package metadata, one component-owned Twig template, SCSS, print SCSS, JavaScript runtime, story, three EC demo data files, test, snapshots, and EC website usage/code/accessibility docs. Table owns one Twig template and imports no component Twig. Story `prepareData` only removes headers and forces `simple` when `header` is false; default, zebra, multi-header, and sortable data are the supported story shapes. Runtime exists only for sortable tables and uses table root `data-ecl-table`/`data-ecl-auto-init="Table"`, sortable header `data-ecl-table-sort-toggle`, sort label attributes, runtime-generated buttons/icons, `aria-sort`, row order attributes, and `data-ecl-auto-initialized`; runtime-generated state is intentionally excluded from static HTML. HTML covers caption present/absent, header/body, enhanced mobile cell labels, simple, zebra, multi-header `rowspan`/`colspan`/`scope="colgroup"`/IDREF/grouped-cell branches, sortable root and header hooks, extension table and row classes/attributes, no-header story output, headers-only, rows-only, empty root-only output, and all 15 non-default EC color modes. Twig sync passed with byte-for-byte `cmp` against the tag. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused table runtime/selector checks, package dry-run inclusion, JS syntax checks for `index.js` and both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The compiled CSS has no literal `ecl-table__cell--group` selector; tagged SCSS styles grouped mobile cells through `data-ecl-table-header-group`, which is present in CSS and examples. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
