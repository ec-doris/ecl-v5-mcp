...
- Phase: none
- Last completed: `link`
- Next component: `list-illustration`
- Next action: change `list-illustration` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Link completed 2026-07-15. Preflight passed: outer tree and linked checkout status outputs were clean, `europa-component-library` is a symlink, and tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`. Tagged inventory has README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, demo data, tests, and snapshots; no EC website component pages are present in the tag. Twig sync passed for `components/link.html.twig`. Reusable HTML covers plain/default output, standalone, primary, primary-highlight, primary-neutral, secondary, secondary-neutral, secondary-inverted, compiled primary-highlight-inverted, branded and inverted booleans, external link icon output, icon before/after/two-icon output, icon-only indicator output, `aria-label`, `id` label ID, extra classes, valued and valueless attributes, absence of link runtime hooks, delegated indicator runtime hooks, and all 15 non-default EC color modes. Excluded no-icon `hide_label` because README/story document that it requires an icon. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, focused link selector/branch checks, color-mode set check, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
