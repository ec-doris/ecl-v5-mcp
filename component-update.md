...
- Status: ready
- Phase: none
- Last completed: `picture`
- Next component: `popover`
- Next action: change `popover` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Picture completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged package contains README, package metadata, one component-owned Twig template, SCSS, demo data, tests, and snapshots. No EC website picture docs were listed by the tagged tree. Audit found no story file, JavaScript runtime, auto-init, or component-owned imports. Twig defines `picture.img.src`, `picture.img.alt`, `picture.image_anchor`, `picture.sources[].src/media/type`, `lazy`, `zoom`, `extra_classes`, `extra_image_classes`, and `extra_attributes`. Breakpoint media aliases map to xs `(min-width: 415px)`, s `(min-width: 480px)`, m `(min-width: 768px)`, l `(min-width: 996px)`, xl `(min-width: 1140px)`, and xxl `(min-width: 1368px)`; other media strings pass through. Root extra classes append to `ecl-picture`; valued and valueless extra attributes render on root, though tagged tests only cover valued attributes. `zoom` appends `ecl-picture--zoom`; `lazy` adds `loading="lazy"` to the image. `image_anchor` renders an image style whenever it is not empty, including default-like `center`; previous local Twig had drifted by suppressing `center`. Empty alt renders `alt=""`; missing alt omits the attribute. Sources can omit any of `srcset`, `media`, and `type`; empty source objects still render `<source>`. Empty input renders root `<picture class="ecl-picture"></picture>`. SCSS and compiled CSS/print CSS include `.ecl-picture img`, `--ecl-image-anchor`, and `.ecl-picture--zoom`; compiled JS has no Picture component runtime or hooks. Reusable HTML covers default responsive output, media alias mapping, source `srcset`/`media`/`type` omissions, zoom, lazy loading, image anchor, image classes, root extension hooks, empty alt, missing alt, image without src, sources without image, empty source object, empty image object, empty input, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused selector/runtime checks, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
