...
- Status: ready
- Phase: none
- Last completed: `file-upload`
- Next component: `form-group`
- Next action: change `form-group` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: File-upload completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; outer project and linked checkout status outputs were clean before this pass; tagged inventory has README, one Twig template, JavaScript, SCSS, print SCSS, two demo data files, tests, snapshot, package metadata, and no EC website docs. Twig sync passed for `components/file-upload.html.twig` against tagged v5.0.1. Audit found component-owned runtime `FileUpload` with auto-init name `FileUpload`, root input hook `data-ecl-file-upload-input`, group selector `[data-ecl-file-upload-group]`, button selector `[data-ecl-file-upload-button]`, list selector `[data-ecl-file-upload-list]`, label attributes `data-ecl-file-upload-label-choose` and `data-ecl-file-upload-label-replace`, runtime-selected item classes, and runtime-only `data-ecl-auto-initialized` excluded from static HTML. Package depends on `@ecl/dom-utils` and `@ecl/event-manager`; no component-owned EC website docs exist. Compiled CSS contains file-upload input/button/list/item selectors; print CSS hides only input/button/list roots; compiled JS bundles contain `FileUpload`, group/button/list selectors, label attributes, and item rendering. Reusable HTML covers default required single upload, multiple upload, disabled, invalid, extra classes, valued and valueless attributes, empty id/name/button labels, runtime selected-file list shape, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused FileUpload selector/hook/state checks, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax checks, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
