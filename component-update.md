...
- Status: ready
- Phase: none
- Last completed: `ordered-list`
- Next component: `page-header`
- Next action: change `page-header` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Ordered-list completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer and linked worktrees reported no short-status entries, and the tagged package contains README, package metadata, one component-owned Twig template, SCSS, print SCSS, four demo data files, story, tests, and snapshots; no EC website docs were present under `src/website/src/pages/ec/components/ordered-list`. Audit found no component JavaScript. Tagged Twig defines `items`, `variant`, `extra_classes`, and `extra_attributes`; it renders only when `items` is non-empty, recurses nested lists without propagating variants or extension hooks, and preserves raw item labels for links. Variants are `no-marker` and `divider`; extra classes and valued/valueless attributes apply to the root only. Tagged story mutates only the first item label and can wrap demo output in `.ecl-container`; the grid wrapper is story-only, not component API. Tagged snapshots cover default, no-marker, divider, extra classes, and valued extra attributes. Local compiled CSS contains ordered-list root, item, nested, `--no-marker`, and `--divider` selectors; no ordered-list runtime hooks are present or required. Local link output can represent the raw `ecl-link` labels used in tagged link demos. Twig sync passed for `components/ordered-list.html.twig`. Reusable HTML covers default text, nested lists, linked raw labels, divider and no-marker variants, extra classes, valued and valueless attributes, empty-input/no-output contract, no-runtime evidence, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused selector/extension checks, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
