...
- Status: ready
- Phase: none
- Last completed: `spinner`
- Next component: `splash-page`
- Next action: change `splash-page` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Spinner completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer and linked checkout status output was empty, and tagged source lists README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, demo data, test, and snapshots. No EC website spinner docs were present in the tagged tree. Twig sync passed with byte-for-byte `cmp` against the tag. HTML covers primary/inverted variants, s/m/l sizes, text present/absent, centered and non-centered roots, visible and hidden roots, visible and hidden overlay output, valued and valueless extra attributes, the tagged empty/default hidden output, the upstream extra-class omission, no JS runtime state, and all 15 non-default EC color mode names as intentionally dropped `extra_classes`. Tagged Twig has an upstream typo that drops `extra_classes` (`_css_class` instead of `_css_classes`), including the color mode classes; exact Twig was preserved and the rendered omission was recorded. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused spinner CSS/print selector checks, no compiled JS runtime selectors, package dry-run inclusion, JS syntax checks for `index.js` and both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
