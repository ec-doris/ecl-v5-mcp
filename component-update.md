...
- Status: ready
- Phase: none
- Last completed: `page-summary`
- Next component: `pagination`
- Next action: change `pagination` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Page-summary completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, upstream package and EC docs are present, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged package contains README, package metadata, one component-owned Twig template, EC SCSS and print SCSS, demo data, story, tests, and snapshots; EC website usage/accessibility docs are empty front matter and code docs only embed the Storybook playground. Audit found Twig defines `id`, `title`, `description`, `icon`, `extra_classes`, and `extra_attributes`; it imports only `@ecl/icon/icon.html.twig`, and the local icon template matches tagged output for the decorative and accessible span branches used here. Default `id` is random, so static examples use explicit IDs; root is always a `<section class="ecl-page-summary" id="..." aria-labelledby="...-title">`, even when the title is absent, creating an upstream IDREF edge case. Header renders when title or icon is present; title renders an `h2` with `id="{id}-title`; icon renders a medium ECL icon with `ecl-page-summary__icon`; description renders raw content and Storybook's `demo_list` branch supplies an ECL unordered list string. Extra classes append to root; valued and valueless extra attributes render on root, though tagged tests cover valued attributes only. SCSS/print define root/header/title/description styles and nested list styling. Compiled CSS and print CSS include page-summary selectors; compiled JS bundles have no page-summary runtime, auto-init, or hooks. Reusable HTML covers default title/decorative icon/paragraph, no-icon title, accessible icon, raw unordered-list description, title-only output, custom ID, extra classes, valued and valueless root attributes, and all 15 non-default EC color modes. Title-absent description-only, icon-only, and empty upstream outputs are intentionally excluded from reusable HTML because tagged Twig emits `aria-labelledby` values with no matching title id. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused selector/runtime checks, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
