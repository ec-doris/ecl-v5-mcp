...
- Status: ready
- Phase: none
- Last completed: `carousel`
- Next component: `category-filter`
- Next action: change `category-filter` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Carousel completed 2026-07-14. `components/carousel.html.twig` is byte-identical to `v5.0.1:src/components/carousel/carousel.html.twig` and SHA-256 `022954c8535530dbbe180e81fe8fb25db28a59b19f2ae85b908659a0113b6b4b`; reusable HTML SHA-256 `1839d73873b2f320f7ab0508b6d84023fe2ed9d4ebc53465e7ac5fbcc5c7ec8b` has no Twig and covers default multi-slide image/credit/video output, one-slide output, small/medium/large banner sizes, full-width-in-container story form, omitted `sr_description`, custom role/play/pause/counter labels, valued and valueless root attributes, custom root classes, delegated banner hooks, empty-input no-root contract, and all 15 non-default EC color modes. Carousel imports `banner` and `button`; those separate rows were not changed. Tagged Twig does not emit the `data-ecl-carousel` attribute shown in EC docs, so reusable HTML follows Twig and excludes it. Runtime-only `data-ecl-auto-initialized` is intentionally excluded. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused Carousel hook/modifier/runtime-exclusion assertions, JS syntax checks, Jest smoke with `--passWithNoTests`, package dry-run inclusion, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
