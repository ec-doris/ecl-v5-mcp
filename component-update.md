...
- Next component: `link`
- Next action: change `link` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Label completed 2026-07-15. Preflight passed: outer tree and linked checkout status outputs were clean, `europa-component-library` is a symlink, and tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`. Tagged inventory has README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, demo data, tests, snapshots, and EC website accessibility/code/usage docs. Twig sync passed for `components/label.html.twig`. Reusable HTML covers default low output, medium/high/highlight variants, extra classes, valued and valueless attributes, tagged empty-label output, absence of runtime auto-init hooks, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, focused label selector/variant checks, color-mode set check, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax checks, non-Twig whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
