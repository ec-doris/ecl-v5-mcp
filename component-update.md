...
- Status: ready
- Phase: none
- Last completed: `featured-item`
- Next component: `file`
- Next action: change `file` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Featured item completed 2026-07-14. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; tagged inventory has README, one Twig template, SCSS, print SCSS, story, demo data, tests, snapshot, and package metadata. Outer status had pre-existing tracker changes from starting this row and prior component work; linked checkout status output was clean. Audit found no component-owned JavaScript/runtime and no EC website component docs at `src/website/src/pages/ec/components/featured-item`. Package depends on `link` and `media-container`; local dependency output is compatible for the used link types/icons/attributes and picture media container branch. Compiled CSS and print CSS contain featured-item selectors/modifiers. `components/featured-item.html.twig` is byte-identical to tagged v5.0.1 source (`cmp` passed; SHA-256 `604a860132ed54cf7d3f36f875c204b65fd3a7798060da5dc1a2b1e557a84d7e`). Reusable HTML covers default static media, micro-title/title/description/standalone link/media, no-media, center horizontal and vertical alignment, dynamic media, media right, default button link, highlight default link, highlight highlighted link, highlight button links in light and dark color modes, deprecated `link_highlighted` output, title absent with no `aria-describedby`, media-only edge shape, extension classes/attributes, and all 15 non-default EC color modes. Excluded: raw story control `show_media` because it only deletes `media_container`; EC website docs absent; constructor/runtime options absent. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused featured-item selector/link/media checks, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax checks, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
