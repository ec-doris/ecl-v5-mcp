...
- Status: ready
- Phase: none
- Last completed: `select`
- Next component: `separator`
- Next action: change `separator` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Select completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged tree lists README, package metadata, one component-owned Twig template, JavaScript runtime, SCSS, print SCSS, single and multiple demo data, tests, and snapshots; no EC website docs were listed by the initial tree command. Audit found one component-owned Twig file. Local Twig was missing v5.0.1 `multiple_counter_text` and `multiple_submit` branches and is now byte-identical to the tag. Twig defines root `<select class="ecl-select">`, optional `id`, `name`, `required`, `disabled`, extra classes, valued/valueless extra attributes, default `data-ecl-auto-init="Select"`, optgroup and standalone option branches, selected/disabled option states, and a delegated tertiary icon-only button inside `.ecl-select__icon`. Multiple select adds `multiple`, `data-ecl-select-multiple`, default/clear/close/counter labels, optional select-all, optional submit, and optional search/no-results attributes. Tagged tests/snapshots cover default, extra classes, extra attributes, required, disabled, invalid when delegated through form-group, and multiple. Demo data covers optgroups, disabled option, selected single option, standalone option, and full multiple labels. Runtime initializes on `data-ecl-auto-init="Select"` and reads multiple-select data attributes to build generated dropdown UI; static examples do not author runtime-generated `data-ecl-auto-initialized` or generated multiple-select DOM. SCSS defines select container width/state classes, hidden container, icon, active state, multiple dropdown, search, select-all, counter, and toolbar; print CSS hides `.ecl-select` and `.ecl-select__icon`. Local delegated button/icon/form-group output can render the pinned shapes used here. HTML covers default, optgroups, standalone/selected/disabled options, required/disabled/invalid form-group states, small/medium/large delegated widths, select extra classes, valued and valueless attributes, full multiple labels including counter and submit, compact multiple without select-all/search, empty select edge output, and all 15 non-default EC color modes. The duplicate `data-ecl-auto-init` merge in tagged Twig for multiple select is represented as one valid static attribute because tagged snapshots normalize it to one attribute. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion, focused select selector/branch/runtime checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
