...
- Status: ready
- Phase: none
- Last completed: `site-header`
- Next component: `skip-link`
- Next action: change `skip-link` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Site header completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the site-header start checkpoint edit. Tagged site-header tree lists README, package metadata, two component-owned Twig templates, EC and EU SCSS, dropdown SCSS, print SCSS, JavaScript runtime, EC and EU stories, EC/EU demo data, tests, and snapshots; no EC website docs were listed by the initial tree command. Twig sync passed for `site-header.html.twig` and `site-header-language-switcher.html.twig`. HTML covers root hooks, extra classes/attributes, logo sizes, no-logo edge output, custom action link and overlay branches, login states, language overlay/category/item branches, search branches, notification close/no-close branches, banner/site-name/CTA branches, menu/mega-menu root modifiers, tagged empty/root-only output, no authored runtime `data-ecl-auto-initialized`, runtime selector evidence, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion, focused site-header hook/selector checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
