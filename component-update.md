...
- Status: ready
- Phase: none
- Last completed: `splash-page`
- Next component: `spotlight`
- Next action: change `spotlight` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Splash page completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer status showed only this pass's tracker change, linked checkout status output was empty, and tagged source lists README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, EC/EU demo data, test, snapshots, and EC website usage/code/accessibility docs. No component-owned JavaScript exists; no `SplashPage` runtime hook is present in compiled bundles. Dependencies are `link` and `picture`; local Twig can render the standalone link and picture output used here. Twig sync passed with byte-for-byte `cmp` against the tag. HTML covers default EC logo and language output, extension hooks, desktop-only logo fallback, omitted title/category headings, 1/2/4 item count modifiers, EU/non-EU branch combinations, active links in both groups, no-items and empty/root-only edge output, and all 15 non-default EC color modes. The tagged print SCSS has no count-modifier selectors for single/two/four item categories; those modifiers were validated against screen CSS only. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused splash-page selector/runtime checks, package dry-run inclusion, JS syntax checks for `index.js` and both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
