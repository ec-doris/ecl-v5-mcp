...
- Phase: none
- Last completed: `quiz`
- Next component: `radio`
- Next action: change `radio` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Quiz completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged package contains README, package metadata, two component-owned Twig templates, SCSS, print SCSS, JavaScript runtime, story, default and poll demo data, tests, snapshots, and EC website docs. Audit found dependency imports for `icon`; local icon template supports the pinned output shapes used here. Twig defines root `id`, `variant`, title/description, background/full-width, pager labels, skip text, items, extra classes, and extra attributes; cards define category/result labels, question/answer content, answer title, correct/incorrect and chosen-option labels, options, icons, flip text/icon, back text, card extra classes/attributes, and reveal/poll branches. Story `prepareData` only sets background/full-width and forces background when full-width is true. Runtime requires `data-ecl-auto-init="Quiz"`, slider, card, flip, poll input, pager, dots, counter, question/answer ID, and chosen-option hooks; it adds initialized state, generated dots/counter, disabled pager state, card flipped/correct/selected classes, and aria-labelledby at runtime. Upstream hardcodes `id="ecl-quiz__dot-template"`, so the reusable HTML keeps exactly one full quiz root to avoid duplicate IDs; additional root/card contract fragments cover branches that would otherwise require duplicating that ID. HTML covers poll and reveal card shapes, header/content branches, background, full-width inner container, root/card extension hooks, skip text first-item behavior, answer-title present/absent, result icons and option icons, chosen-option attributes, exact runtime hooks, absence of runtime-generated state, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused selector/runtime checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
