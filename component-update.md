...
- Status: ready
- Phase: none
- Last completed: `radio`
- Next component: `range`
- Next action: change `range` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Radio completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged package contains README, package metadata, two component-owned Twig templates, SCSS, print SCSS, default and binary demo data, tests, and snapshots; no component-owned JavaScript runtime or EC website docs were listed. Audit found no imports in the component-owned Twig. `radio-group.html.twig` only loops over `items` and propagates `name`, `invalid`, `binary`, and `required` to each radio button; it computes group extra classes/attributes but prints no group wrapper in v5.0.1, so those inputs intentionally have no static output. `radio-button.html.twig` defines `id`, `name`, `value`, `disabled`, `invalid`, `required`, `checked`, `binary`, `helper_text`, `label`, per-button extra classes, and per-button extra attributes. SCSS defines disabled, checked, invalid, focus-visible, helper text, and binary styles; print CSS hides `.ecl-radio`; there is no radio JavaScript or auto-init hook. HTML covers default multi-item output, required propagation, checked, disabled, rich label content, invalid, invalid+disabled, helper text, binary and binary+invalid, per-button extra class and valued/valueless attributes, minimal no-id/no-name/no-label output, tagged empty group output, absence of runtime hooks, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused radio selector/structure checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. The first structural validation attempt failed only because `COMPONENT` was not exported to inline Node; rerun with `export COMPONENT` passed. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
