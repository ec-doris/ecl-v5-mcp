...
- Status: ready
- Phase: none
- Last completed: `timeline`
- Next component: `tooltip`
- Next action: change `tooltip` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Timeline completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer status showed no output before this pass's tracker edit, and linked checkout status output was empty. Tagged source lists `.npmignore`, README, package metadata, `timeline.html.twig`, `timeline-set.html.twig`, SCSS, print SCSS, JavaScript, default and set demo data, story, test, snapshots, and EC website accessibility/API/code/usage docs. Upstream audit completed: timeline has two component-owned Twig files; `timeline.html.twig` owns the ordered-list root, headline, item, collapsed-item, toggle-button include, `data-ecl-timeline`, and `data-ecl-auto-init="Timeline"` hooks; `timeline-set.html.twig` owns the wrapper and delegates each item to timeline. Twig sync passed byte-for-byte for both local Twig files. Runtime toggles `aria-expanded`, `data-ecl-timeline-expanded`, button label text, focus on first collapsed item, and `--ecl-timeline-label-height`; examples do not author runtime-generated `data-ecl-auto-initialized`, expanded state, or constructor-only options. Compiled assets contain `ecl-timeline`, `ecl-timeline-set`, headline/collapsed/toggle selectors, `data-ecl-timeline`, `data-ecl-timeline-button`, `data-ecl-label`, and `Timeline`. Coverage ledger completed: examples cover no-hidden-items, headline present/absent, label/title/content optional branches including content with delegated link markup, negative `hide.to` with bottom items, positive/no-bottom toggle, positive `hide.to`, no items root-only output, timeline set with inherited and item color mode, set empty output recorded by Twig behavior, extra classes, valued and valueless attributes, all runtime hooks, no authored runtime state, and all 15 non-default EC color modes as compact roots. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused timeline selector/runtime checks, package dry-run inclusion, JS syntax checks for both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
