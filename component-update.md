...
- Next component: `content-block`
- Next action: change `content-block` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Checkbox completed 2026-07-14. `components/checkbox-group.html.twig` and `components/checkbox-item.html.twig` are byte-identical to their tagged v5.0.1 sources, with SHA-256 `5dcafba77b9cab692b5e44bb2247bfaec181c2d617e9c10eccba36a5d399058f` and `5074de49bd9accdac28b4bbb8ebef6cf0f467b574849ee4d3018f60bdb85f900`; reusable HTML SHA-256 `a3fd4e7d286949292c87c2041e4458368add9488e8e6b8dd73cbd2e7a6510c71` has no Twig and covers multi-item group output, checked and unchecked inputs, disabled and invalid modifiers, rich label content, single invalid group feedback, required and optional label text with ARIA notes, helper text and disabled helper text, item and group extension hooks with valued and valueless attributes, imported icon output, and all 15 non-default EC color modes. Empty `items` render no markup and are intentionally excluded. Checkbox has no JavaScript auto-init or runtime state. Validation passed: Twig identity for both Twig files, no-Twig HTML, Prettier check, structural/IDREF check, focused checkbox coverage assertions, JS syntax check, Jest smoke with `--passWithNoTests`, package dry-run inclusion, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
