...
- Status: ready
- Phase: none
- Last completed: `popover`
- Next component: `quiz`
- Next action: change `quiz` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Popover completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the start checkpoint edit. Tagged package contains README, package metadata, one component-owned Twig template, SCSS, print SCSS, JavaScript runtime, story, demo data, tests, snapshots, and EC website docs. Audit found imports of `button` and `link`; local dependency templates support the pinned output shapes used here. Twig defines `id`, `toggle`, `close`, `content`, `extra_classes`, and `extra_attributes`. Root always renders `class="ecl-popover"` plus extra classes and `data-ecl-auto-init="Popover"` plus valued/valueless extra attributes. Toggle is rendered as link when `toggle.link` is not empty, otherwise as button, and always receives `aria-controls`, valueless `data-ecl-popover-toggle`, `aria-expanded="false"`, and `ecl-popover__toggle` plus any toggle extra classes. Close button is optional; when present it is forced to tertiary neutral button markup with `type="button"`, `ecl-popover__close`, and valueless `data-ecl-popover-close`. Content is arbitrary HTML or text inside `.ecl-popover__content`. Empty input still renders a button include, random id, empty content, and no close button; static empty-input output is excluded because the random id cannot be reusable. Story `prepareData` only changes toggle label, content, and optional indicator state; if custom content is non-empty it deletes an unused `links` property. Runtime requires `.ecl-popover`, `.ecl-popover__container`, first child scrollable wrapper, toggle `aria-controls`, `data-ecl-popover-toggle`, optional `data-ecl-popover-close`, and auto-init `Popover`; it adds/removes `data-ecl-auto-initialized`, toggles `hidden` and `aria-expanded`, focuses close/toggle, closes on Escape/outside click, and adds placement classes `--top`, `--bottom`, `--left`, `--right`, and push classes at runtime. Reusable HTML covers default button with close and no-JS fallback, link trigger, no-close branch, arbitrary rich content, icon-only accessible toggle, indicator toggle, root extension hooks with valued and valueless attributes, toggle extra class, authored placement classes, exact runtime hooks, no runtime-initialized state, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion using `npm pack --dry-run --json`, focused selector/runtime checks, Jest smoke with `--passWithNoTests`, JS syntax check, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
