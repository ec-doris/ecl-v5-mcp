...
- Status: ready
- Phase: none
- Last completed: `date-block`
- Next component: `datepicker`
- Next action: change `datepicker` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Date-block completed 2026-07-14. `components/date-block.html.twig` is byte-identical to tagged v5.0.1 source; reusable HTML covers default date output, ongoing/cancelled/past variants, abbreviated and plain month markup, date-time present and absent branches, extra root class, valued and valueless attributes, documented empty/day-only no-output guard, no-runtime compiled JS check, compiled CSS and print selectors, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, focused date-block coverage and selector assertions, package dry-run inclusion, JS syntax check, Jest smoke with `--passWithNoTests`, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
