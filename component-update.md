...
- Status: ready
- Phase: none
- Last completed: `unordered-list`
- Next component: `video`
- Next action: change `video` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: `unordered-list` completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer status showed no output before this pass's tracker edit, and linked checkout status output was empty. Tagged source lists README, package metadata, SCSS, print SCSS, Twig, story, tests, snapshots, and four demo data files; no EC website API/code/usage/accessibility docs were listed by preflight. Upstream audit completed: Twig renders nothing for empty `items`; root `ul` class starts as `ecl-unordered-list`, optional `variant` appends `ecl-unordered-list--no-marker` or `ecl-unordered-list--divider`, optional `extra_classes` appends root classes, and optional `extra_attributes` supports valued and valueless attributes. Each non-empty item renders `li.ecl-unordered-list__item`, raw `label`, and an optional nested list; nested lists are rendered by the macro with empty variant/classes/attributes, so authored nested variant classes are excluded even though SCSS styles nested lists inside variant roots by descendant selectors. Story `prepareData` only rewrites the first item label and optionally wraps the story in grid demo content, which is not component API. Snapshots cover default text, extra classes, extra attributes, no-marker, and divider. Compiled CSS/print CSS contain `ecl-unordered-list`, `ecl-unordered-list__item`, `ecl-unordered-list--no-marker`, and `ecl-unordered-list--divider`; print CSS also contains `ecl-unordered-list__item--first` but tagged Twig never emits it, so it is not authored. Compiled JS has no unordered-list runtime. Coverage ledger completed: examples cover default text with nested lists, link labels, both variants with nested lists remaining plain, root extra classes, valued and valueless root attributes, empty-items no-output contract as a comment, no runtime hooks, and all 15 non-default EC color modes as compact root examples. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused selector/runtime/nested-variant checks, package dry-run inclusion, authored-file trailing whitespace check, Jest smoke with `--passWithNoTests`, JS syntax checks for both compiled bundles, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
