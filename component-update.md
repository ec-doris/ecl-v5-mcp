...
- Status: ready
- Phase: none
- Last completed: `datepicker`
- Next component: `description-list`
- Next action: change `description-list` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Datepicker completed 2026-07-14. `components/datepicker.html.twig` is byte-identical to tagged v5.0.1 source; reusable HTML covers autoinit present and absent, explicit id/name, stable exclusion of random id output, widths `s`/`m`/`l`, default value present and absent, placeholder present and absent, min/max, required, disabled, invalid, alternate first day of week, extra root class, valued and valueless attributes, runtime hook omission of `data-ecl-auto-initialized`, compiled JS Datepicker/default-value/placeholder evidence, compiled CSS width selectors, print selector, and all 15 non-default EC color modes. Tagged v5.0.1 has no EC website page or story file for `datepicker`; tagged tests for `extra_group_classes` and `extra_label_classes` produce no markup because Twig has no such parameters. Upstream emits `ecl-datepicker--invalid`, but compiled CSS styles invalid state through `.ecl-datepicker[invalid]`. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused datepicker branch and selector assertions, package dry-run inclusion, JS syntax check, Jest smoke with `--passWithNoTests`, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
