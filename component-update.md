...
- Status: ready
- Phase: none
- Last completed: `expandable`
- Next component: `fact-figures`
- Next action: change `fact-figures` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Expandable completed 2026-07-14. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; outer status only had tracker/expandable edits; linked checkout status output was clean. Tagged inventory has README, one Twig template, JS, SCSS, print SCSS, story, demo data, tests, snapshot, package metadata, and EC website usage/API/code/accessibility pages. `components/expandable.html.twig` is byte-identical to tagged v5.0.1 source. Reusable HTML covers default paragraph content, rich HTML content, dropdown extra classes, root extra classes, valued and valueless root attributes, button option passthrough via extra toggle class, empty content, empty labels/content edge output, valid static ID/ARIA controls for every non-edge root, runtime hooks, omission of runtime-generated expanded/initialized state, and all 15 non-default EC color modes as extra root classes. Tagged `expanded: true` story/test input does not alter Twig output; it is only a Storybook play/runtime interaction. Missing random-ID branch is intentionally excluded because reusable static HTML must keep stable IDREFs. EC docs warn against nested expandables, so nested output is not included. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused selector/runtime hook checks, package dry-run inclusion, JS syntax check, Jest smoke with `--passWithNoTests`, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
