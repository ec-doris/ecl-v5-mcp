...
- Status: ready
- Phase: none
- Last completed: `separator`
- Next component: `site-footer`
- Next action: change `site-footer` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Separator completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, and both outer and linked repositories had no status output before the separator start checkpoint edit. Tagged separator tree lists README, package metadata, one component-owned Twig template, SCSS, print SCSS, story, tests, and snapshots; no EC website docs were listed by the initial tree command. Audit found one component-owned Twig file and no JavaScript runtime. Twig renders an `<hr>` with base class `ecl-separator`, optional `extra_classes`, and optional iterable `extra_attributes` supporting valued and valueless attributes. Tests/snapshots cover default, extra classes, and valued extra attributes; Twig additionally has a valueless extra-attribute branch. Story demonstrates separators between unordered, ordered, and description list content. SCSS defines border-top and zero margin; print SCSS keeps the top border and adds print spacing. Compiled CSS contains `.ecl-separator`; compiled JS bundles do not contain separator runtime hooks. HTML covers default, extra classes, valued and valueless attributes, story list context, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion, focused separator selector/branch/no-runtime checks, JS syntax check, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
