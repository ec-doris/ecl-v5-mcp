...
- Status: ready
- Phase: none
- Last completed: `slogan-ticker`
- Next component: `social-media-follow`
- Next action: change `social-media-follow` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Slogan-ticker completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, the linked checkout had no status output, and outer status only showed this tracker after the start checkpoint edit. Tagged source lists README, package metadata, one component-owned Twig template, JS, SCSS, print SCSS, story, demo data, test, snapshot, and EC website docs. Audit complete: Twig renders a root `div` with `data-ecl-slogan-ticker="true"`, `data-ecl-auto-init="SloganTicker"`, `id`, optional `data-ecl-slogan-ticker-autoplay="false"`, optional EC `color_mode` class, extension classes/attributes, a slider viewport/track with one `li` per item content, and delegated tertiary icon-only play/pause buttons. Runtime requires the root, slider, play/pause hooks, and at least one slide to initialize; with no slides it hides controls and returns false. JavaScript adds `data-ecl-auto-initialized`; do not author it. SCSS/compiled CSS contain `ecl-slogan-ticker` selectors; compiled JS/ESM contain `SloganTicker` and hook selectors. README item `link` and `external` fields are ignored by v5.0.1 Twig and story data, so link/external item examples are intentionally excluded. HTML covers default multi-item, autoplay false, custom labels, extension classes, valued/valueless extension attributes, empty-items edge output, the `color_mode` parameter, no authored runtime `data-ecl-auto-initialized`, runtime hooks, and all 15 non-default EC color modes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set check, package dry-run inclusion, focused selector/runtime checks, JS syntax checks for `index.js` and both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
