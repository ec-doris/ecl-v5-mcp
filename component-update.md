...
- Next action: change `carousel` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Card completed 2026-07-14. `components/card.html.twig` is byte-identical to `v5.0.1:src/components/card/card.html.twig` and SHA-256 `2dfe15d43fa750211c48731b54031cb0b2e69c7155345c5301be94e6298f054e`; reusable HTML SHA-256 `87c8e8610f028718438c065cd238eb158ba0b5c33f7f404777865a312ec604df` has no Twig and covers the audited ledger: picture with `data-ecl-picture-link`, labels with `labels_aria`, multiple primary meta, linked title with `data-ecl-title-link`, description, vertical secondary meta with icon, description-list shapes, no-picture output, single label and single primary/secondary meta `div` branches, plain title, horizontal secondary meta, custom root class, valued and valueless root attributes, `picture_zoom`, missing-image output, empty/root-only output, and all 15 non-default EC color modes. Card imports `content-block` and `picture`; those separate rows were not changed. Runtime-only `data-ecl-auto-initialized` is intentionally excluded. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused Card hook/branch/runtime-exclusion assertions, JS syntax checks, Jest smoke with `--passWithNoTests`, package dry-run inclusion, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
