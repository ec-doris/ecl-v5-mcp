...
- Status: ready
- Phase: none
- Last completed: `blockquote`
- Next component: `breadcrumb`
- Next action: change `breadcrumb` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Blockquote completed 2026-07-14. `components/blockquote.html.twig` is byte-identical to `v5.0.1:src/components/blockquote/blockquote.html.twig` and SHA-256 `91623ad9b2e6932010ce9b6ba63323bf024129646bb8dd680e3bbb741765cb05`; reusable HTML has no Twig and covers the audited ledger: default/empty color mode, all 15 non-default EC modes, citation/author, `lang`, picture absent/present/responsive/decorative, extra class, valued and valueless attributes, and tagged empty-input/root-only output. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, package dry-run inclusion, focused Blockquote structure/class assertions, Jest smoke, JS syntax checks, whitespace and diff checks. The npm pack `.npmignore` warning is expected. `ecl-blockquote__body` and `ecl-blockquote__picture` are markup classes from tagged Twig/picture output but do not compile to standalone CSS selectors; focused CSS checks were limited to selectors present in the pinned compiled CSS.
