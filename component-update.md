...
- Status: ready
- Phase: none
- Last completed: `fact-figures`
- Next component: `featured-item`
- Next action: change `featured-item` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Fact figures completed 2026-07-14. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; tagged inventory has README, one Twig template, SCSS, print SCSS, story, demo data, tests, snapshot, package metadata, and EC website code/usage/accessibility pages. Outer status had pre-existing tracker changes from starting this row and prior component work; linked checkout status output was clean. Audit found no component-owned JavaScript/runtime; compiled CSS contains fact-figures selectors. Twig imports `icon` and `link`; local subcomponent output is compatible for decorative icons and standalone/source links used here. `components/fact-figures.html.twig` is byte-identical to tagged v5.0.1 source (`cmp` passed; SHA-256 `0fa7340e40051a9d02d688eb2ac5fc3ec55ea3dc90d2e86a8743b1883a00e6cb`). Reusable HTML covers columns 1-4, centered layout, medium and large value font, icon display on/off, `l` and `2xl` icon sizes, value-with-title, value-only, title-only, descriptions, `is-first`, single and multiple items, no sources, one plain source, one linked source, multiple mixed sources with label, internal and external view-all links, extension classes/attributes, empty root output, and all 15 non-default EC color modes. EC accessibility docs say icons are always decorative, so no accessible-icon branch is included. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused fact-figures selector/source/link checks, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax checks, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. `ecl-fact-figures--col-1` and `ecl-fact-figures__sources-label` are valid Twig output but have no dedicated compiled CSS selector, so focused validation checks them in HTML only.
