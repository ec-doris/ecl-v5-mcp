...
- Status: ready
- Phase: none
- Last completed: `form-group`
- Next component: `gallery`
- Next action: change `gallery` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Form-group completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; tagged inventory has README, one Twig template, SCSS, print SCSS, story, tests, snapshot, package metadata, and no component-owned JavaScript or EC website docs. Local `components/form-group.html` and `components/form-group.html.twig` already existed while row was `not started`; reconciled them against the tag. Audit found form-group is a wrapper/orchestrator for text/search, select, textarea, datepicker, radio, checkbox, range, rating-field, and file input components. Twig sync passed for `components/form-group.html.twig` against tagged v5.0.1. Reusable HTML covers text/search/select/textarea/datepicker/radio/checkbox/range/rating/file input shapes, div and fieldset roots, hidden/invalid/disabled labels, required/optional indicators with aria labels, helper and invalid messages, grouped hidden helper/invalid text, range and file wrapper hooks including upstream `data-ecl-autoinit="FileUpload"` typo, extra label/root classes, valued and valueless attributes, empty root edge output, and all 15 non-default EC color modes. A static `id` was added to the datepicker custom element so the upstream label target is valid in reusable HTML. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused FormGroup selector/hook/state checks, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax checks, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
