...
- Next component: `blockquote`
- Next action: change `blockquote` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Banner completed 2026-07-14. `components/banner.html.twig` is byte-identical to `v5.0.1:src/components/banner/banner.html.twig` and SHA-256 `cd15487f56dbdcd5b066f27b83e3348b0dcf6e6102c8f2d2d77abaec6d7613ed`; reusable HTML has no Twig and covers the audited ledger. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, package dry-run inclusion, focused Banner hook/class assertions, Jest smoke, JS syntax checks, whitespace and diff checks. The npm pack `.npmignore` warning is expected. EC docs mention `.ecl-banner--fullwidth`, but tagged Twig and SCSS use `.ecl-banner--full-width`; preserve the tagged contract.
