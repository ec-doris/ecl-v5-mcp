...
- Status: ready
- Phase: none
- Last completed: `description-list`
- Next component: `expandable`
- Next action: change `expandable` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Description-list completed 2026-07-14. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; outer and linked checkout status outputs were clean; tagged inventory has `.npmignore`, README, one Twig template, JS, SCSS, print SCSS, story, two demo data files, tests, snapshot, and package metadata. No EC website page exists for `description-list` at v5.0.1. `components/description-list.html.twig` is byte-identical to tagged v5.0.1 source. Reusable HTML covers empty no-root output, vertical and horizontal roots, runtime hook presence and absence, visible item/more-label attributes, collapsible list hooks, string and array terms, term and definition omission branches, string/block and text-array definitions, link, link-inline, tag, taxonomy plain and linked values, external and icon links, extra root classes, valued and valueless attributes, runtime omission of generated see-more/hidden/last-visible/tabindex/auto-initialized state, and all 15 non-default EC color modes as extra classes. Local link, icon, and tag output used by tagged snapshots is compatible enough for the pinned dependency shapes. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused selector/runtime hook checks, package dry-run inclusion, JS syntax check, Jest smoke with `--passWithNoTests`, whitespace check, and `git diff --check`. Print CSS lacks runtime hidden/last-visible selectors while screen CSS and both JS bundles include them; print CSS hides the generated see-more control. The npm pack `.gitignore` fallback warning is expected.
