...
- Status: ready
- Phase: none
- Last completed: `tooltip`
- Next component: `unordered-list`
- Next action: change `unordered-list` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: Tooltip completed 2026-07-16. Preflight passed: `europa-component-library` is a symlink, tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`, outer status showed no output before this pass's tracker edit, and linked checkout status output was empty. Tagged source lists README, package metadata, SCSS, print SCSS, JavaScript, story, and EC website accessibility/API/code/usage docs; no tagged Twig, tests, snapshots, or demo data exist. Upstream audit completed: HTML is story-authored; initialization belongs on a container with `data-ecl-auto-init="Tooltip"` and triggers use `data-ecl-tooltip` or `data-ecl-tooltip-inverted`, optionally valueless with `title` fallback. Runtime creates sibling `.ecl-tooltip` popups, `aria-describedby`, `role="tooltip"`, `aria-hidden`, optional `popover="manual"`, inline positioning, `ecl-tooltip--bottom`, `ecl-tooltip--inverted`, and `data-ecl-auto-initialized`; examples do not author those runtime outputs. Runtime events are hover, focus, Escape, resize, and scroll; constructor options do not create HTML attributes. Compiled assets contain `ecl-tooltip`, `ecl-tooltip--bottom`, `ecl-tooltip--inverted`, both trigger attributes, and `Tooltip`. Coverage ledger completed: examples cover default and inverted style triggers, button and link triggers, valued data attributes, valueless data attributes with title fallback, scoped container auto-init, an empty-content inert edge case, no authored popup/state, and all 15 non-default EC color modes as compact initialized containers. Validation passed: empty Twig identity arrays, no-Twig HTML, Prettier check, structural/ID check, color-mode set check, focused tooltip selector/runtime checks, package dry-run inclusion, JS syntax checks for both compiled bundles, Jest smoke with `--passWithNoTests`, and `git diff --check`. The npm pack `.gitignore` fallback warning and Jest no-tests result are expected. Laravel full-suite testing was not applicable because this repository has no `composer.json` or `artisan`.
