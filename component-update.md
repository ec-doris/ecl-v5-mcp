...
- Phase: none
- Last completed: `file`
- Next component: `file-upload`
- Next action: change `file-upload` to `started` here before auditing its tagged v5.0.1 source or editing its files
- Evidence and warnings: File completed 2026-07-15. Preflight passed: `europa-component-library` is a symlink; tag `v5.0.1` resolves to `eceefe9468e44f7ce9c57801c91c4c0c057548b4`; tagged inventory has README, three Twig templates, JavaScript, SCSS, print SCSS, story, demo data, tests, snapshot, package metadata, and EC website docs. Outer status had pre-existing tracker changes from starting this row and prior component work; linked checkout status output was clean. Audit found component-owned runtime `FileDownload` with auto-init name `FileDownload`, toggle selector `[data-ecl-file-translation-toggle]`, container selector `[data-ecl-file-translation-container]`, and runtime-only `data-ecl-auto-initialized` excluded from static HTML. Package depends on button, description-list, icon, label, link, picture, and dom-utils; existing local dependency markup is compatible for used button/link/icon/label/picture/description-list branches. Compiled CSS, print CSS, and JS bundles contain file selectors/hooks. Twig sync passed for `components/file.html.twig`, `components/file-translations.html.twig`, and `components/file-translation-item.html.twig` against tagged v5.0.1. Reusable HTML covers default variant, icon branch, plain title, detail meta, language, meta ID/ARIA references, download link, no translation/no auto-init, translation auto-init/toggle/list, `hreflang`, per-item meta IDs, default download attribute inheritance, item override true/false, `aria-expanded=false`, thumbnail with picture, thumbnail without picture, linked title, description, single label, multiple labels, taxonomy list, extra classes, valued/valueless attributes, download extra attributes/classes, and all 15 non-default EC color modes. Excluded: Storybook `show_preview` placeholder because it is story-only DOM mutation after render; constructor-only runtime options because they do not add static attributes; `data-ecl-auto-initialized` because runtime sets it. Validation passed: Twig identity, no-Twig HTML, Prettier check, structural/IDREF check, color-mode set diff, focused file selector/hook/ARIA/branch checks, package dry-run inclusion, Jest smoke with `--passWithNoTests`, JS syntax checks, whitespace check, and `git diff --check`. The npm pack `.gitignore` fallback warning is expected.
